# Gebruiksvoorwaarden Welvio

**Versie:** 1.0  
**Ingangsdatum:** 6 juni 2026  
**Aanbieder:** QuikNova, bredewold9@hotmail.com

---

## 1. Acceptatie van de voorwaarden

Door Welvio te installeren of te gebruiken, verklaart u akkoord te gaan met deze gebruiksvoorwaarden ("Voorwaarden"). Bent u het niet eens met deze Voorwaarden, gebruik de app dan niet.

---

## 2. Beschrijving van de dienst

Welvio is een mobiele applicatie voor het bijhouden van persoonlijk vermogen, kasstromen en financiële voortgang. De app biedt een gratis basisversie en een betaald Pro-abonnement.

**De app biedt uitdrukkelijk geen financieel advies.** Alle berekeningen en inzichten zijn indicatief en dienen slechts als persoonlijk overzicht. QuikNova is niet aansprakelijk voor financiële beslissingen die op basis van de app worden genomen.

---

## 3. Abonnementen en betalingen

### 3a. Gratis versie

De gratis versie biedt toegang tot basisfunctionaliteiten met bepaalde limieten (maximaal 3 categorieën, beperkte inzichten). Gebruik van de gratis versie is kosteloos en onbeperkt in tijd.

### 3b. Proefperiode (7 dagen gratis)

Nieuwe gebruikers kunnen eenmalig een gratis proefperiode van 7 dagen voor Pro activeren. Na afloop van de proefperiode wordt de Pro-toegang automatisch beëindigd, tenzij u een abonnement afsluit. Er wordt geen betaling gevraagd tijdens de proefperiode.

### 3c. Pro-abonnement

Het Pro-abonnement is beschikbaar als:
- **Maandabonnement:** €2,99 per maand
- **Jaarabonnement:** €24,99 per jaar

Prijzen zijn inclusief toepasselijke belastingen en kunnen per land variëren vanwege lokale btw-tarieven. De feitelijke afrekenprijs wordt weergegeven vóór aankoop in de App Store of Google Play.

### 3d. Betaling en verlenging

Betalingen verlopen uitsluitend via **Apple App Store** of **Google Play Store**. Uw abonnement verlengt automatisch aan het einde van elke abonnementsperiode, tenzij u het ten minste 24 uur vóór het einde van de lopende periode opzegt.

### 3e. Opzegging

U kunt uw abonnement te allen tijde opzeggen via:
- **iOS:** Apparaatinstellingen → Apple ID → Abonnementen → Welvio
- **Android:** Google Play Store → Account → Abonnementen → Welvio

Na opzegging behoudt u Pro-toegang tot het einde van de betaalde periode.

### 3f. Herroepingsrecht (EU-consumenten)

Op grond van de Richtlijn Consumentenrechten (2011/83/EU) heeft u als EU-consument het recht een digitale dienst binnen 14 dagen te herroepen. Door de proefperiode te starten of een abonnement af te sluiten en direct gebruik te maken van de Pro-functionaliteiten, verzoekt u ons de dienst onmiddellijk te leveren en doet u afstand van uw herroepingsrecht overeenkomstig artikel 16 sub m van de Richtlijn.

---

## 4. Gratis proefperiode — aanvullende voorwaarden

- De proefperiode is beschikbaar voor nieuwe gebruikers die nog geen Pro-abonnement hebben gehad.
- Per Apple ID of Google-account geldt één proefperiode.
- Tijdens de proefperiode zijn alle Pro-functionaliteiten beschikbaar.
- Na afloop van de proefperiode worden geen kosten in rekening gebracht; uw account keert terug naar de gratis versie.

---

## 5. Gebruik van de app

### 5a. Toegestaan gebruik

U mag de app gebruiken voor persoonlijk, niet-commercieel gebruik. Het is niet toegestaan de app te kopiëren, reverse engineeren, decompileren of te distribueren zonder onze uitdrukkelijke schriftelijke toestemming.

### 5b. Verboden gebruik

Het is verboden de app te gebruiken voor:
- Het invoeren van gegevens van derden zonder hun toestemming
- Commerciële doeleinden zonder licentie
- Handelingen die in strijd zijn met Nederlands of internationaal recht

---

## 6. Intellectueel eigendom

Alle rechten op de app, inclusief broncode, ontwerp, logo's en teksten, berusten bij QuikNova. Niets in deze Voorwaarden geeft u eigendomsrechten op de app of de inhoud daarvan.

---

## 7. Gegevens en privacy

Het gebruik van de app is onderworpen aan ons Privacybeleid, dat integraal onderdeel uitmaakt van deze Voorwaarden. Wij verwijzen u naar ons Privacybeleid voor informatie over de verwerking van uw gegevens.

---

## 8. Aansprakelijkheidsbeperking

Voor zover wettelijk toegestaan:

- QuikNova is **niet aansprakelijk** voor financiële verliezen of schade die voortvloeien uit het gebruik van, of vertrouwen op, de berekeningen en inzichten in de app.
- De app wordt aangeboden "zoals ze is" ("as is") zonder garanties omtrent nauwkeurigheid, volledigheid of geschiktheid voor een specifiek doel.
- Onze maximale aansprakelijkheid is beperkt tot het bedrag dat u in de 12 maanden voorafgaand aan de schade aan ons heeft betaald.

---

## 9. Wijzigingen in de dienst en Voorwaarden

Wij behouden het recht de app of de Voorwaarden te wijzigen. Bij wezenlijke wijzigingen informeren wij u via een melding in de app. Voortgezet gebruik na de ingangsdatum van gewijzigde Voorwaarden geldt als acceptatie.

---

## 10. Beëindiging

Wij kunnen uw toegang tot de app beëindigen bij schending van deze Voorwaarden. U kunt de Voorwaarden te allen tijde beëindigen door de app te verwijderen en uw abonnement op te zeggen.

---

## 11. Toepasselijk recht en geschillen

Op deze Voorwaarden is **Nederlands recht** van toepassing. Geschillen worden voorgelegd aan de bevoegde rechtbank in Nederland. Als consument kunt u ook gebruik maken van het Europees ODR-platform (ec.europa.eu/odr) voor buitengerechtelijke geschillenbeslechting.

---

## 12. EULA (End User License Agreement) — Apple vereiste

Voor gebruikers die de app via de Apple App Store hebben gedownload, geldt aanvullend de standaard Apple EULA, beschikbaar op apple.com/legal/internet-services/itunes/dev/stdeula. In geval van strijdigheid prevaleren deze Gebruiksvoorwaarden voor zover de Apple EULA dit toestaat.

---

## 13. Contact

**QuikNova**  
E-mail: bredewold9@hotmail.com  
Land: Nederland

---

*Deze gebruiksvoorwaarden zijn opgesteld conform Nederlands recht en de toepasselijke EU-regelgeving, waaronder de Richtlijn Consumentenrechten (2011/83/EU) en de AVG.*
